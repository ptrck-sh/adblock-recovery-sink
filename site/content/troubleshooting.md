...
| The page shows the recovery dialog without trying fallback hosts | A loader host is not rewritten to the sink, and the resolver answers it with a block page whose certificate the device trusts, such as the NextDNS block page. The blocked script counts as loaded, and the page only tries fallback hosts after a load error. | Rewrite every loader host in the bundled profile, including the `*.` entries. |
