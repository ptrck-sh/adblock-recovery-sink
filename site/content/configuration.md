...
| `toast.enabled` | `false` | `ARS_TOAST_ENABLED` | |
| `toast.details` | `false` | `ARS_TOAST_DETAILS` | |
...
`hosts` and `profiles` are comma-separated in environment variables and flags. When `hosts` is unset, the service derives it from enabled profiles, filters it to the root and intermediate CA name constraints, and reports skipped hosts on `/status`. It fails if none remain. Explicit hosts remain strict: every configured host must be provided by an enabled profile and permitted by the CA. `hostname` is an optional public name shown on `/install`.
