...
The metrics registry exposes `ars_requests_total{profile,result}`, `ars_tls_handshake_failures_total{reason}`, `ars_cert_cache_entries`, `ars_issuer_expiry_timestamp_seconds`, and `ars_profile_info{profile,version}`. Request results are `matched`, `unknown_host`, `unknown_path`, or `method_not_allowed`; handshake reasons are `unknown_sni` or `other`.
